...
class TransactionModelTests(TestCase):
    ...
    def setUp(self):
        self.sender_account = Account.objects.create(cpf="12345678901", email=f"sender_{uuid.uuid4()}@example.com", balance=100.00)
        self.receiver_account = Account.objects.create(cpf="12345678902", email=f"receiver_{uuid.uuid4()}@example.com", balance=50.00)
    ...
    def test_transaction_str_method(self):
        ...
        expected_str = f"Transferência de {self.sender_account} para {self.receiver_account} no valor de R$10.00 em {transaction.timestamp}"
